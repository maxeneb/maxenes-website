...
const ProjectSlider = () => {
  function NextArrow({ onClick }) {
    return (
      <button
        onClick={onClick}
        className="cursor-pointer absolute right-4 top-1/2 -translate-y-1/2 z-10 p-2 bg-sage-dark/20 text-sage-dark hover:bg-sage-medium hover:text-cream border border-sage-medium rounded-full backdrop-blur-sm transition duration-200 hover:scale-110"
      >
  ...
  function PrevArrow({ onClick }) {
    return (
      <button
        onClick={onClick}
        className="cursor-pointer absolute left-4 top-1/2 -translate-y-1/2 z-10 p-2 bg-sage-dark/20 text-sage-light hover:bg-sage-medium hover:text-cream border border-sage-medium rounded-full backdrop-blur-sm transition duration-200 hover:scale-110"
      >
        <ChevronLeft size={20} />
      </button>
    );
    ...
    <div className="w-full max-w-6xl mx-auto px-4">
      <style jsx global>{`
        ...
        .custom-dots li button:before {
          content: "" !important; /* remove numbers */
      ...
      <Slider {...settings}>
        {projects.map((project, index) => (
          <div key={project.id}>
            <div className="grid md:grid-cols-2 bg-cream min-h-[450px]">
              {/* Image Section */}
              ...
              <div className="p-6 md:p-10 flex flex-col justify-center">
                ...
                <div className="absolute top-4 right-4 flex gap-2 z-30">
                  <a
                    ...
                    className="p-2 bg-sage-dark/30 border border-sage-medium rounded-full bg-sage-medium text-cream hover:bg-sage-dark transition duration-200 hover:scale-110"
                    title="View Code"
                  >
                    <Github size={18} className=""/>
                  </a>
                </div>
                <h3 className="text-1xl md:text-2xl font-bold text-sage-dark mb-3">
                  {project.title}
                </h3>
                <p className="text-sage-dark text-sm mb-3">
                  {project.description}
                </p>
                <p className="text-sage-medium text-sm mb-6">
                    {project.category}
                </p>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.tech.map((tech, i) => (
                      <span
                        key={i}
                        className="px-3 py-1 bg-sage-light/30 text-sage-dark rounded-full text-sm font-medium border border-sage-medium/30 transition duration-300 hover:scale-110"
                      >
                        {tech}
                      </span>
                    ))}
